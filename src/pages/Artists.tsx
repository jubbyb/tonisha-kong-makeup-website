...
export default function Artists() {
  ...
  return (
    <div className="container mx-auto p-6 max-w-5xl">
      ...
      <p className="text-center text-base-content/60 mb-10">Choose an artist to view their profile and book an appointment.</p>

      {artists.length === 0 ? (
        <div className="text-center text-base-content/50 py-20">No artists available at this time.</div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {artists.map((artist) => (
            <div key={artist.id} className="card bg-base-100 shadow-md hover:shadow-xl transition-shadow">
              ...
              <div className="card-body">
                ...
                {artist.specialties && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {artist.specialties.split(',').map((s) => (
                      <span key={s} className="badge badge-outline badge-sm">{s.trim()}</span>
                    ))}
                  </div>
                )}
                {artist.bio && (
                ...
                <div className="card-actions justify-end mt-2">
                  <button className="btn btn-primary btn-sm" onClick={() => navigate(`/artists/${artist.id}`)}>
                    View &amp; Book
                  </button>
                </div>
              </div>
