...
export default function IndustryHub() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/service-catalog?industry=${slug}`).then(
        (r) => r.json() as Promise<CatalogCategory[]>,
      ),
    ])
      .then(([ind, arts, cat]) => {
  ...
  if (notFound || !industry) {
    return (
      ...
      >
        <p style={{ color: 'var(--ink-2)' }}>Industry not found.</p>
        <Link
          to="/industries"
          style={{
            color: 'var(--accent)',
  ...
  const previewServices = catalog
    .flatMap((cat) =>
      cat.subcategories.flatMap((sub) =>
        sub.services.map((svc) => ({ ...svc, category: cat.name })),
      ),
    )
    .slice(0, 6);
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', padding: '4rem 2rem' }}>
        ...
        {artists.length === 0 ? (
          <p style={{ color: 'var(--ink-2)', fontSize: '0.9rem' }}>
            No artists available for this industry yet.
          </p>
        ) : (
          <div
            style={{
